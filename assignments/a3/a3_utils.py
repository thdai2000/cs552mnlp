...
class GeneratorForCausalLM():
    ...
    def prepare_next_inputs(
        ...
        """
        return {'input_ids': torch.cat((model_inputs['input_ids'], new_token_id.unsqueeze(0)), dim=1).to(model_inputs['input_ids'].device),
                'attention_mask': torch.cat((model_inputs['attention_mask'], torch.tensor([[1]])), dim=1).to(model_inputs['attention_mask'].device)}
